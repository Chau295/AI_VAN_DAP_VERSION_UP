...
class Command(BaseCommand):
    ...
    async def process_main_question(self, message):
        ...
        try:
            ...
            session = await sync_to_async(ExamSession.objects.prefetch_related('questions').get)(pk=session_id)

            # Check if exam is still open
            exam_group = session.exam_session_group_id
            ...
            ordered_questions = list(session.questions.all())
            question_order = next((index + 1 for index, item in enumerate(ordered_questions) if item.pk == question.pk), None)
            ...
            raw_transcript = await self.process_audio_and_transcribe(reply_channel, chunks,
                                                                     whisper_prompt=question.question_text)
            if raw_transcript is None or raw_transcript == "": return
            rephrased = await rephrase_text_with_chatgpt(raw_transcript, question.question_text, self.openai_client)
            final_score, feedback = await score_student_answer_with_openai(rephrased, question.question_text, self.openai_client)
